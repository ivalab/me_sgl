...
from utils import getObjectSeg, getObjectInfo, construct_initial_state, get_groundtruth_perception, target_object_select
...
def main(args):
    ...
    for instance_folder_name in sorted(os.listdir(os.path.join(args.root, args.task_type, args.level))):
        # ToDo: delete it later, for temporary debugging
        instance_folder_name = '0'
        ...
        gt_object, gt_object_info, gt_subject, gt_subject_info = get_groundtruth_perception(controller,
                                                                                            ...
                                                                                            OBJECT_LIST,
                                                                                            OBJECT_ATTRIBUTES)
        # gt_object, gt_object_seg, gt_subject, gt_subject_seg = get_groundtruth_perception(controller, scene_info)
        ...
        # detect_object_list = []
        # for label in labels:
        #     detect_object_list.append(OBJECT_LIST[label - 1])
        ...
        with open(os.path.join(folder_path, 'pred_pddl_goal_state.txt'), 'r') as f:
            pddl_goal_state_pr = f.readline().split('\n')[0].split()
        # ToDo: delete later
        # pddl_goal_state_pr = ['cut', 'apple', 'knife']
...
if __name__ == "__main__":
    ...
    parser.add_argument('--level', type=str, default='easy',
                        help='Level of the task, can be easy, median, hard1 and hard2')
    # ToDo: delete it later
    parser.add_argument('--comm_data_root', type=str, default='../comm',
                        help='Root folder for saving communication data between task learning and other modules')
